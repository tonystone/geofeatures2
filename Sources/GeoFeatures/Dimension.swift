...
///
/// - Remarks: The dimension of empty sets are donated as `.empty` (-1).  The dimension of non-empty
...
public enum Dimension: Int {
    ...
    /// `Point`s and `Point` collections
    case zero = 0

    /// `Curve` types and `Curve` collections
    case one  = 1

    /// `Surface` types and `Surface` collection
    case two  = 2
}
