...
class WKTReaderCoordinate2DFloatingPrecisionCartesianTests: XCTestCase {
    ...
    private typealias WKTReaderType = WKTReader
    private var wktReader = WKTReaderType(precision: Floating(), coordinateSystem: Cartesian())
    ...
    func testReadDataNotConvertableUsingUTF8() {
        ...
        XCTAssertThrowsError(try wktReader.read(data: input)) { error in
            ...
            if case WKTReader.Errors.invalidData(let message) = error {
    ...
    func testReadInvalidGeometry() {
        ...
        XCTAssertThrowsError(try wktReader.read(string: input)) { error in
            if case WKTReader.Errors.unsupportedType(let message) = error {
                XCTAssertEqual(message, expected)
            } else {
    ...
    func testReadPointInvalidCoordinateNoSpace() {
        ...
        XCTAssertThrowsError(try wktReader.read(string: input)) { error in
            if case WKTReader.Errors.unexpectedToken(let message) = error {
                XCTAssertEqual(message, expected)
            } else {
                XCTFail("Wrong error thrown")
            }
    ...
    func testReadPerformancePolygonCalifornia() {
        let reader = WKTReader(precision: Fixed(scale: 100000))
